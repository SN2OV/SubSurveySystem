...
public class MainController {
    ...
    public String index(@ModelAttribute("loginUser") User inputUser, ModelMap modelMap, HttpSession session,HttpServletResponse httpResponse,HttpServletRequest request) {
        ...
        Cookie cokRole = CookieUtils.getCookieByName(request, "role");
        if (cokLoginName != null && cokLoginPwd != null && cokRole!= null && cokLoginName.getValue() != "" && cokLoginPwd.getValue() != ""&&cokRole.getValue()!=""){
            String loginName = cokLoginName.getValue();
            String loginPwd = cokLoginPwd.getValue();
            String role = cokRole.getValue();
...
//        }else{
            ...
            if(password == null || !password.equals(inputUser.getPassword())){
                ...
                CookieUtils.addCookie(httpResponse,"role",role,loginMaxAge);
                if(role.equals("管理员")){
                    List<User> userList = usersRepository.findAll();
                    modelMap.addAttribute("userList", userList);
                    return "admin/user/main";
